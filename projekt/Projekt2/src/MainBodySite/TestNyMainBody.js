...
function Lower({ grades }) {
    ...
    const handleMouseEnter = (studUser) => {
        setHoveredRow(studUser); //spara studentents studUser
    };

    const handleMouseLeave = () => {
        setHoveredRow(null); //rensar när man slutar hovra över studentens namn
    };
                        ...
                        grades.map((grade, index) => (
                            <tr key={index}>
                                <td
                                    ...
                                    onMouseEnter={() => handleMouseEnter(grade.studUser)}
                                    onMouseLeave={handleMouseLeave}
                                >
                                    {grade.firstName} {grade.lastName}
                                    {hoveredRow === grade.studUser && (
                                        <span className="tooltip">{hoveredRow}</span>
                                    )}
                                </td>
